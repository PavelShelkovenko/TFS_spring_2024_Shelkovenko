package com.pavelshelkovenko.tfs_spring_2024_shelkovenko.presentation.screens.chat.actor
...
import com.pavelshelkovenko.tfs_spring_2024_shelkovenko.presentation.screens.chat.actor.repository.FakeCharRepositoryWithErrors
import com.pavelshelkovenko.tfs_spring_2024_shelkovenko.presentation.screens.chat.actor.repository.FakeChatRepository
import com.pavelshelkovenko.tfs_spring_2024_shelkovenko.presentation.screens.chat.actor.repository.FakeStreamRepository
...
class ChatActorTest {
    ...
    private val getTopicsForStreamByIdUseCase = GetTopicsForStreamByIdUseCase(FakeStreamRepository())
    ...
    fun `LoadMessagesFromCache command should emit LoadMessagesFromCache event`() = runTest {
        ...
        val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
        // When
    ...
    fun `LoadMessagesFromCache command should emit MinorError event if error occurs`() = runTest {
        // Given
        val actor = ChatActor(repository = fakeCharRepositoryWithErrors, getTopicsForStreamByIdUseCase)
        // When
        val actual = actor.execute(ChatCommand.LoadMessagesFromCache(streamName, topicName))
        // Then
        assertEquals(
    ...
    fun `LoadMessagesFromNetwork command should emit LoadMessagesFromNetwork event`() = runTest {
        // Given
        val messages = fakeChatRepository.getMessagesFromNetwork(
            streamName,
            topicName,
            anchor = "anchor",
            numBefore = 1,
    ...
    fun `LoadMessagesFromNetwork command should emit MinorError event if error occurs`() = runTest {
        ...
        // Then
        assertEquals(
            ChatEvent.Internal.Error::class.java.name,
    ...
    @Test
    fun `RegisterForChatEvents command should emit RegistrationForChatEventsDataReceived event`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `GetReactionEvents command should emit ReactionEventsDataReceived and GetReactionLongPollingData event`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `GetReactionEvents command should emit GetReactionLongPollingData event when error occurs`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeCharRepositoryWithErrors, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `GetMessageEvents command should emit MessageEventsDataReceived and GetMessageLongPollingData event`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `GetMessageEvents command should emit GetMessageLongPollingData event when error occurs`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeCharRepositoryWithErrors, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `LoadPagingNewerMessages command should emit LoadPagingNewerMessages and LoadingPagingDataFinished event`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `LoadPagingNewerMessages command should emit MinorError and LoadingPagingDataFinished event when error occurs`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeCharRepositoryWithErrors, getTopicsForStreamByIdUseCase)
            // When
            ...
            // Then
            assertEquals(
                ChatEvent.Internal.PagingError::class.java.name,
                actual.first().javaClass.name
            )
            assertEquals(
                ChatEvent.Internal.LoadingPagingDataFinished,
                actual.last()
    ...
    @Test
    fun `LoadPagingOlderMessages command should emit LoadPagingOlderMessages and LoadingPagingDataFinished event`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeChatRepository, getTopicsForStreamByIdUseCase)
            // When
    ...
    @Test
    fun `LoadPagingOlderMessages command should emit MinorError and LoadingPagingDataFinished event when error occurs`() =
        runTest {
            // Given
            val actor = ChatActor(repository = fakeCharRepositoryWithErrors, getTopicsForStreamByIdUseCase)
            // When
